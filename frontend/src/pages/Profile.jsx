...
export default function Profile() {
  ...
  const changePw = async (e) => {
    e.preventDefault();
    if (pwForm.new_password!==pwForm.confirm) return toast.error(t('كلمتا المرور غير متطابقتين'));
    setSaving(true);
    try {
  ...
  return (
    <div className="font-arabic max-w-lg space-y-5" dir={dir}>
      <h1 className="text-xl font-bold text-slate-800">{t('الملف الشخصي')}</h1>
      ...
      <div className="bg-white border border-[#EEF2FF] rounded-2xl p-5 flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-[#4F46E5] text-sm">{t(roleLabel[user?.role])}</p>
      ...
      <div className="flex gap-1 bg-white p-1 rounded-xl w-fit border border-[#EEF2FF]">
        {[['profile',<User size={14}/>, 'البيانات'],['password',<Lock size={14}/>, 'كلمة المرور']].map(([tk,icon,l])=>(
          <button key={tk} onClick={()=>setTab(tk)}
            className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-bold transition-all ${tab===tk?'bg-[#4F46E5] !text-white':'text-slate-500 hover:text-slate-800'}`}>
            {icon}{t(l)}
          </button>
        ))}
      </div>
      ...
      {tab==='password' && (
        <form onSubmit={changePw} className="bg-white border border-[#EEF2FF] rounded-2xl p-5 space-y-4">
          <div><label className={lbl}>{t('كلمة المرور الحالية')}</label>
          ...
          <div><label className={lbl}>{t('كلمة المرور الجديدة')}</label>
          ...
          <div><label className={lbl}>{t('تأكيد كلمة المرور الجديدة')}</label>
