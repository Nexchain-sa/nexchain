  ...
  return (
    <div className="font-arabic space-y-5" dir="rtl">
      <div className="flex items-center gap-2">
        ...
        <h1 className="text-xl font-bold text-slate-800">الصفقات</h1>
        <span className="text-xs text-slate-400">— تتبّع مراحل كل صفقة</span>
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl border py-14 text-center" style={{ borderColor: '#E5E7EF' }}>
          ...
          <p className="text-slate-400">لا توجد صفقات بعد. تبدأ الصفقة بإنشاء فاتورة ثم طلب تمويلها.</p>
        </div>
      ) : (
              ...
              <div key={d.id} className="bg-white rounded-2xl border p-5" style={{ borderColor: '#E5E7EF' }}>
                <div className="flex items-center justify-between gap-3 flex-wrap mb-5">
                  <div>
                    ...
                    <p className="text-lg font-bold mt-0.5" style={{ color: '#4F46E5' }}>{money(d.amount)} <span className="text-xs text-slate-400">ر.س</span></p>
                  </div>
                  {cancelled
                    ? <span className="text-xs font-bold px-3 py-1.5 rounded-lg" style={{ background: '#FEE2E2', color: '#DC2626' }}>ملغاة</span>
                    : <span className="text-xs font-bold px-3 py-1.5 rounded-lg" style={{ background: '#EEF2FF', color: '#4F46E5' }}>{STAGES[active].label}</span>}
                  {d.inst_total > 0 && (
                    ...
                    return (
                      <React.Fragment key={st.key}>
                        <div className="flex flex-col items-center gap-1.5" style={{ minWidth: 56 }}>
                          ...
                          <span className="text-[11px] font-bold text-center" style={{ color: current || done ? '#1E293B' : '#94A3B8' }}>{st.label}</span>
                        </div>
                        {i < STAGES.length - 1 && (
